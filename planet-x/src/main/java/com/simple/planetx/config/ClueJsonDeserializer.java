...
@JsonComponent
public class ClueJsonDeserializer extends JsonDeserializer<List<MapJson.Space>> {
    ...
    public List<MapJson.Space> deserialize(JsonParser p, DeserializationContext ctxt) throws IOException, JacksonException {
        ...
        LinkedHashMap<String, List<LinkedHashMap<String, Integer>>> map = p.getCodec().treeToValue(treeNode, LinkedHashMap.class);
        map.forEach((k,v) -> v.forEach(vv -> result.addAll(transToSpaceList(vv))));
        return result;
    }

    private List<MapJson.Space> transToSpaceList(Map<String, Integer> map) {
        List<MapJson.Space> result = new ArrayList<>();
        map.forEach((k,v) -> {
