...
public class AskAnswerUtil {
    ...
    private SpeedBootMessage<?> message;
    ...
    public SpeedBootMessage<?> askSelectScore(@NotNull SpeedBootPlayer player, @NotNull AtomicInteger playTimes){
        lock.lock();
        try {
            ...
            log.debug("询问玩家{}选择分数还是继续",player.getUser().getUsername());
            messagePublishUtil.sendToRoomUser(String.valueOf(player.getUser().getId()), roomId, message);
            boolean await = playDiceCondition.await(waitTimeOutToCloseRoom, TimeUnit.MINUTES);
            if(!await){
                throw new SpeedBootException(SpeedBootExceptionEnum.CLOSE_FOR_OPERATE_TIMEOUT);
            }
